...
{
    internal static class RemoveImageTemplatePertanyaan
    {
        [Authorize]
        public static void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet("TemplatePertanyaan/{id}/RemoveImage", async (string id, ISender sender, HttpContext context, TokenValidator tokenValidator) =>
            {
                ...
                Result result = await sender.Send(new RemoveImageTemplatePertanyaanCommand(
                    Guid.Parse(id)
                ));
